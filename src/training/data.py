...
def get_wds_dataset(
    ...
    if resampled:
        ...
        ), "--train_data_upsampling_factors is only supported when sampling with replacement (together with --dataset-resampled)."
    ...
    if args.pretokenized_file_suffix is None and tokenizer is None:
        raise RuntimeError(
            "No tokenizer given, and --pretokenized_file_suffix flag not set."
        )
    if args.pretokenized_file_suffix is not None and tokenizer is not None:
        raise RuntimeError(
    ...
    def random_frame(video, is_train=is_train):
        ...
        start_frame = np.random.randint(0, len(video)) if is_train else 0

        assert args.return_n_frames > 0, "return_n_frames must be > 0"
        if args.return_n_frames > 1:
            end_frame = start_frame + args.return_n_frames * args.frame_stride
            frames_list = [
                to_pil_image(frame)
                for frame in torch.permute(video, (0, 3, 1, 2))[
                    start_frame : end_frame : args.frame_stride
                ]
            ]
            return frames_list
        else:
    ...
    if args.return_n_frames > 1:
        ...
        def possibly_batched_preprocess_img(frames):
            assert isinstance(frames, list)
            if isinstance(frames, list):
                return torch.stack([preprocess_img(frame) for frame in frames])

    else:

        def possibly_batched_preprocess_img(frame):
            assert isinstance(frame, Image.Image)
            return preprocess_img(frame)
    ...
    if args.videos_only:
        ...
        pipeline.extend(
            [
                ...
                wds.map_dict(image=random_frame),
                wds.map_dict(
                    image=possibly_batched_preprocess_img,
            ...
            [
                ...
                wds.map_dict(
                    ...
                    text=lambda p: torch.tensor(
                        [int(t) for t in p.split(",")], dtype=torch.int64
                    ),
                ),
            ]
        )
        ...
        def random_token_range(tokens):
            ...
            if tokens.shape[0] < n_to_sample:
                ...
                if is_train:
                    ...
                    seed = (
                        int(hashlib.sha256(str(tokens).encode("utf-8")).hexdigest(), 16)
                        % 10**8
                    )
